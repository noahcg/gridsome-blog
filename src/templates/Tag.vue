<template>
  <Layout>
    <main>
      <header>
        <div
          class="max-w-xl md:max-w-3xl xl:max-w-4xl flex flex-col-reverse mx-auto text-center pt-20 pb-5 md:pt-30 md:pb-16"
        >
          <p class="eading-normal">
            {{ $page.tag.belongsTo.totalCount }} posts in total
          </p>
          <h1
            class="text-4xl sm:text-5xl md:text-5xl font-sans font-bold mb-2 capitalize"
          >
            {{ titleCase($page.tag.title) }}
          </h1>
          <svg
            class="w-5 sm:w-6 fill-current text-gray-500 mx-auto mb-4"
        ...
        <nav class="absolute top-0 left-0 z-20">
          <g-link
            to="/blog"
            class="text-sm px-4 py-2 text-gray-900 border-gray-400"
            >&larr; Blog</g-link
          >
        </nav>
      </header>
